...
class ReportsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (final g in kGroups) {
      if (english == null && g.name == 'إنجليزي B2 — مسائي أ') english = g;
      if (math == null && g.name == 'رياضيات تاسع — مسائي') math = g;
    }

    if (kGroups.isEmpty || english == null || math == null) {
      return SingleChildScrollView(
        ...
        child: Container(
          ...
          child: Column(children: const [
            Icon(Icons.assessment_outlined, size: 40, color: AppTheme.textSub),
            SizedBox(height: 10),
            Text('لا توجد بيانات تقريرية بعد',
                style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: AppTheme.dark)),
            SizedBox(height: 6),
            Text('بعد إنشاء الدورات وتنفيذ جلسات أو إدخال بيانات فعلية ستظهر هنا التقارير الحية.',
