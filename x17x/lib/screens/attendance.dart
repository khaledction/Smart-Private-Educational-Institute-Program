...
class _AttendanceScreenState extends State<AttendanceScreen> {
  String? _group = kAttendance.isEmpty ? null : kAttendance.first.group;

  @override
  Widget build(BuildContext context) {
    if (kAttendance.isEmpty) {
      return SingleChildScrollView(
        ...
        child: Container(
          ...
          child: Column(children: const [
            Icon(Icons.fact_check_outlined, size: 40, color: AppTheme.textSub),
            SizedBox(height: 10),
            Text('لا توجد بيانات حضور بعد',
                style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: AppTheme.dark)),
            SizedBox(height: 6),
            Text('ابدأ بإنشاء دورة وتسجيل طلاب فعليين، ثم ستظهر هنا جلسات الحضور الحقيقية.',
    ...
    final currentGroup = (_group != null && kAttendance.any((x) => x.group == _group)) ? _group! : kAttendance.first.group;
    final a = kAttendance.firstWhere((x) => x.group == currentGroup);
