                  ...
                  : () async {
                      final ok = await showDialog<bool>(
                            ...
                            builder: (context) => AlertDialog(
                              ...
                              content: const Text('سيتم حذف كل البيانات المحلية نهائيًا: طلاب، دورات، طلبات، وانتظار، مع تفريغ البيانات التشغيلية والمالية الحالية للتجربة. هل تريد المتابعة؟'),
                              actions: [
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          child: const Row(children: [
            ...
            Expanded(
              child: Text(
                'إذا كانت الدورة المطلوبة مكتملة أو مغلقة، تستطيع الإدارة اختيار دورة بديلة مفتوحة أو وضع الطالب على قائمة انتظار المدرس نفسه. وعند توفر مقعد في الدورة المرتبطة سيُضاف الطالب تلقائيًا ويُفتح له إشعار واتساب.',
          ...
          rows: [
            for (final w in store.waiting)
              [
                ...
                ElevatedButton(
                  onPressed: () => onSnack('🔔 هذا الطالب سينتقل تلقائيًا عند توفر مقعد في الدورة المرتبطة، وسيُفتح له إشعار واتساب.'),
                  ...
                  child: const Text('انتقال تلقائي عند التوفر', style: TextStyle(fontSize: 11.5)),
                ),
              ],
          ],
        ),
              ...
              Wrap(spacing: 8, runSpacing: 8, children: [
                ...
                StatusChip(
                  preferred.status == 'closed'
                      ? 'مغلقة'
                      : (preferred.isFull ? 'مكتملة' : (preferred.status == 'running' ? 'جارية' : 'مفتوحة')),
                  color: preferred.status == 'closed'
                      ? AppTheme.danger
                      : (preferred.isFull ? AppTheme.gold : (preferred.status == 'running' ? AppTheme.success : AppTheme.seed)),
                ),
              ]),
              const SizedBox(height: 8),
