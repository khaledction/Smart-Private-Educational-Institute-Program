...
class RegistrationStore extends ChangeNotifier {
  ...
  Future<ActionResult> resetAllLocalData() async {
    try {
      ...
      await refresh();
      await _syncStudentsIfReady();
      return const ActionResult(true, '🧹 تم تفريغ جميع البيانات الحالية: التسجيل والطلاب والدورات والانتظار، وكذلك البيانات التشغيلية والمالية والمحاسبية للتجربة. يمكنك الآن البدء من الصفر.');
    } catch (e) {
  ...
  Future<ActionResult> approveRequestToGroup(int requestId, int groupId, {String? note}) async {
    try {
      ...
      kNotifications.insert(
        ...
        NotificationItem(
          'تم اعتماد التسجيل',
          '${student?.name ?? 'طالب'} سُجّل في ${group.name} مع ${group.teacher}${whatsappOpened ? ' وتم فتح واتساب لإشعاره.' : ''}',
          'الآن',
          'request',
        ),
      );
      ...
      return ActionResult(
        true,
        direct
            ? '✅ تم اعتماد الطلب وتسجيل الطالب في الدورة المطلوبة داخل قاعدة البيانات${whatsappOpened ? '، وتم فتح رسالة واتساب جاهزة له.' : '.'}'
            : '✅ تم اعتماد الطلب وتحويل الطالب إلى دورة بديلة مفتوحة وحُفظ ذلك في القاعدة${whatsappOpened ? '، وتم فتح رسالة واتساب جاهزة له.' : '.'}',
        status: 'معتمد',
      );
    } catch (e) {
  ...
  Future<ActionResult> moveRequestToWaiting(int requestId, {String? note}) async {
    try {
      ...
      final waitingGroupId = preferredGroup?.id;
  ...
  Future<ActionResult> removeStudentFromCourse({
    ...
    try {
      ...
      if (enrollmentRows.isEmpty) {
        return const ActionResult(false, 'هذا الطالب غير مربوط بهذه الدورة حاليًا.');
      }
      ...
      final studentName = studentRows.isEmpty ? 'الطالب' : (studentRows.first['full_name'] as String? ?? 'الطالب');
      ...
      await db.update(
        ...
        whereArgs: [studentId, groupId, 'معتمد'],
      );
  ...
  Future<void> _enrollStudentInGroup(Database db, {required int studentId, required GroupOption group}) async {
  ...
  Future<void> _refreshStudentAcademicState(Database db, int studentId) async {
    final enrollmentRows = await db.rawQuery(
      '''
      SELECT g.subject_name, g.system, g.period
      FROM enrollments_local e
      JOIN v_groups_registration g ON g.id = e.group_id
      WHERE e.student_id = ?
      ORDER BY e.id DESC
      ''',
      [studentId],
    );
    final waitingRows = await db.query(
      'v_waiting_list',
      columns: ['period'],
      where: 'student_id = ?',
      whereArgs: [studentId],
    ...
    if (enrollmentRows.isNotEmpty) {
      ...
      for (final row in enrollmentRows) {
        final subject = (row['subject_name'] as String? ?? '').trim();
      ...
      final latestPeriod = (enrollmentRows.first['period'] as String? ?? 'مسائي').trim();
      await db.update(
        'students',
        {
          'status': 'نشط',
          'preferred_period': latestPeriod.isEmpty ? 'مسائي' : latestPeriod,
          'preferred_system': systems.isEmpty
              ? '—'
              : (systems.length == 1 ? systems.first : 'متعدد'),
          'level': subjects.isEmpty
              ? '—'
              : (subjects.length == 1 ? subjects.first : 'عدة مواد'),
        },
        where: 'id = ?',
        whereArgs: [studentId],
      );
    ...
    if (waitingRows.isNotEmpty) {
      final waitingPeriod = (waitingRows.first['period'] as String? ?? 'مسائي').trim();
      await db.update(
        'students',
        {
          'status': 'قائمة انتظار',
          'preferred_period': waitingPeriod.isEmpty ? 'مسائي' : waitingPeriod,
          'preferred_system': '—',
          'level': '—',
        },
        where: 'id = ?',
  ...
  Future<String> _fillVacanciesFromWaiting(Database db, int groupId, {String? reason}) async {
    final promotedStudents = <String>[];

    while (true) {
      final group = await _groupByIdFromDb(db, groupId);
      if (group == null || !group.isOpenForRegistration || group.isFull) {
        break;
      }
      ...
      final waitingRow = waitingRows.first;
      final waitingId = waitingRow['id'] as int;
      final studentId = waitingRow['student_id'] as int;
      ...
      await db.update(
        ...
        whereArgs: [studentId, subjectId, teacherId, period, 'قائمة انتظار'],
      );
      ...
      final studentName = studentRows.isEmpty ? 'الطالب' : (studentRows.first['full_name'] as String? ?? 'الطالب');
      promotedStudents.add(studentName);
      ...
      await _log(
        ...
        'group',
        groupId,
        'تمت إضافة $studentName تلقائيًا من قائمة الانتظار إلى ${group.name}.${whatsappOpened ? ' وتم فتح واتساب لإشعاره.' : ''}',
      );
      kNotifications.insert(
        0,
        NotificationItem(
          'ترقية من الانتظار',
          'تمت إضافة $studentName تلقائيًا إلى ${group.name}${whatsappOpened ? ' وتم فتح واتساب لإشعاره.' : ''}',
          'الآن',
          'waiting',
        ),
      );
    ...
    if (promotedStudents.isEmpty) return '';
  ...
  String _approvalWhatsAppText({
    ...
    required GroupOption group,
    required bool fromWaiting,
  }) {
    final intro = fromWaiting
        ? 'مرحبًا $studentName، تم نقل طلبك من قائمة الانتظار إلى الدورة التي رغبت بها.'
        : 'مرحبًا $studentName، تم اعتماد طلب تسجيلك بنجاح.';
    return '$intro\nالمادة: ${group.subject}\nالمدرس: ${group.teacher}\nالفترة: ${periodLabel(group.period)}\nالقاعة: ${group.room}\nالدوام: ${group.days.isEmpty ? 'سيحدد لاحقًا' : group.days}\nشكرًا لك.';
  }

  Future<bool> _notifyStudentOnWhatsApp(Database db, {required int studentId, required String message}) async {
    final rows = await db.query(
      'students',
      columns: ['phone'],
      where: 'id = ?',
    ...
    final rawPhone = (rows.first['phone'] as String? ?? '').trim();
  ...
  String _normalizeWhatsAppPhone(String phone) {
    ...
    if (digits.startsWith('09')) return '963${digits.substring(1)}';
